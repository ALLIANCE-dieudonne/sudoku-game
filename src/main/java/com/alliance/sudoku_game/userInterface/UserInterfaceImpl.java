...
import com.alliance.sudoku_game.problemDomain.Coordinates;
...
public class UserInterfaceImpl implements IUserInterfaceContract.View,
  ...
  private static final String SUDOKU = "Fils's Sudoku";
  ...
  private void drawTextFields(Group root) {
    ...
    for (int xIndex = 0; xIndex < 9; xIndex++) {
      for (int yIndex = 0; yIndex < 9; yIndex++ ) {
        double x = xOrigin + xIndex * xAndYDelta;
        double y = yOrigin + yIndex * xAndYDelta;
    ...
    for (int xIndex = 0; xIndex < 9; xIndex ++) {
      for (int yIndex = 0; yIndex < 9; yIndex ++) {
        ...
        if(game.getGameState() == GameState.NEW) {
          if (value.equals("")) {
            tile.setStyle("-fix-opacity: 1;");
            tile.setDisable(false);
          } else {
            tile.setStyle("-fix-opacity: 0.8;");
